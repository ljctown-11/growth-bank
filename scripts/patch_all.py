...
def patch(path, repls):
    ...
    for old, new in repls:
        if old not in s:
            # 调试：打印未命中的 old 前 60 字符
            print('!! MISS in', path, '::', repr(old[:60]))
            raise SystemExit(1)
...
# ---------- water.js ----------
patch('features/tree-garden/water.js', [
('''export function revokeEffortWater(taskId, date){
  ...
  gt.pendingWater = Math.max(0, gt.pendingWater - pts);
  // 注意：不清除 effortGranted[key]。取消勾选当天再勾选时，grantEffortWater 看到记录仍在会返回 false，
  // 不再重复产水（避免「取消→复勾」一天内重复发放）。已浇到树上的水不倒回。
  return true;
}''',
'''export function revokeEffortWater(taskId, date){
  if(date !== getTodayStr()) return false; // D2：仅当天收回
