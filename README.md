...
| **💎 五维徽章** | 学习力/运动力/自控力/探索力/实践力 各 L1–L5 徽章，按积分阈值自动评定，点击放大查看，25 张位图预缓存秒开 |
...
> 注：本地另含 `成长树更新_公众号文章/`、`公众号文章/` 两个私有素材目录，及 `tests/`（单元/QA 测试 + 归档的临时预览页），均不纳入版本库（见 .gitignore）。
```
...
| 版本 | 日期 | 更新内容 |
|------|------|---------|
| v3.2.03 | 2026-07-30 | 新增五维徽章系统（5 维度 × 5 级，25 张 WebP 位图 + 自动评级 + 灯箱放大）；仓库整理（杂项归档 tests/，补 .gitignore）；修复 Service Worker 版本号不一致导致的循环更新提示；修复手机端顶栏换行与成长树显示不全 |
| v3.2.04 | 2026-08-03 | 统一全项目版本号至 v3.2.04（修复 CACHE_NAME 与 SW_VERSION/资源清单不一致导致的误报更新）；徽章放大灯箱光效改为金色旋转放射光芒 |
| v3.2.03 | 2026-07-13 | 成长树蝴蝶随机漫游；临时隐藏「我的成就」徽章墙；版本号机制升级 |
